...
export default function PatientOverview({ onStartQuery }) {
  const [selectedMetric, setSelectedMetric] = useState('ldl');

  const patient = patientData;
  const alert = getMetricAlert(patient);
  const riskStyle = getRiskLevelStyle(patient.cardiovascularRisk.category);

  // Prepare chart data based on selected metric
  const chartData = formatChartData(patient.metrics[selectedMetric].history);
    ...
    >
      ...
      <div className="grid md:grid-cols-2 gap-6">
              ...
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <div
                      className={`flex items-center gap-1 ${getTrendColor('ldl', patient.metrics.ldl.trend)}`}
                    >
                      <TrendingUp className="w-4 h-4" />
                      <span className="text-sm font-semibold">
                        {Math.abs(patient.metrics.ldl.changePercent).toFixed(
                          1
                        )}
                        %
                      </span>
                  ...
                  <TooltipContent>
                    <p>
                      Change from baseline: {patient.metrics.ldl.baseline}{' '}
                      {patient.metrics.ldl.unit}
                    </p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="text-lg">Estradiol</CardTitle>
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <div
                      className={`flex items-center gap-1 ${getTrendColor('estradiol', patient.metrics.estradiol.trend)}`}
                    >
                      <TrendingDown className="w-4 h-4" />
                      <span className="text-sm font-semibold">
                        {Math.abs(
                          patient.metrics.estradiol.changePercent
                        ).toFixed(1)}
                        %
                      </span>
                    </div>
                  ...
                  <TooltipContent>
                    <p>
                      ...
                      {patient.metrics.estradiol.baseline}{' '}
                      {patient.metrics.estradiol.unit}
                    </p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="flex items-baseline gap-2">
                <span className="text-4xl font-bold">
                  {patient.metrics.estradiol.current}
                </span>
                <span className="text-muted-foreground">
                  {patient.metrics.estradiol.unit}
                </span>
                ...
                <ResponsiveContainer width="100%" height={160}>
                  <LineChart
                    data={formatChartData(
                      patient.metrics.estradiol.history
                    )}
                    onClick={() => setSelectedMetric('estradiol')}
                  >
                    <CartesianGrid
                      strokeDasharray="3 3"
                      stroke="oklch(0.922 0 0)"
                    ...
                    <YAxis
                      domain={getChartDomain(
                        formatChartData(patient.metrics.estradiol.history)
                      )}
                      tick={{ fill: 'oklch(0.556 0 0)' }}
                      style={{ fontSize: '12px' }}
                    />
